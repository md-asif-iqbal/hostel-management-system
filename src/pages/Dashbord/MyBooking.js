...
const MyBooking = () => {
  ...
  console.log(id);
  const handleCancel = () => {
    const url = `https://hostel-system.onrender.com/purchase/${id}`;
    fetch(url, {
      method: "DELETE",
      headers: {
  ...
  const url = `https://hostel-system.onrender.com/myitems?email=${email}`;
  const {
    data: myOrders,
    isLoading,
    refetch,
        ...
        <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
          <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            <div className="flex flex-col justify-start items-start bg--primary-content px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full font-mono">
              <p className="text-lg md:text-xl font-semibold leading-6 xl:leading-5 text-white font-mono mb-4">
                Customer’s Cart
              </p>

              {myOrders?.map((item) => (
                <div className="mt-6 md:mt-0 flex justify-start flex-col md:flex-row  items-start md:items-center space-y-4  md:space-x-6 xl:space-x-2 w-full mb-5">
                    ...
                    <div className="lg:flex justify-between items-center  mx-auto lg:space-x-8  w-full">
                      ...
                      {item?.status ? (
                        ...
                        <p className="text-base xl:text-lg  leading-6 text-white">
                          {" "}
                          <label
                            className=" uppercase btn  text-white cursor-pointer btn-primary"
                            onClick={() => setDeletes(item)}
            ...
            <div className="flex justify-center md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
              ...
              <div className="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg--primary-content space-y-6  font-mono">
                <h3 className="text-xl font-semibold leading-5 text-white">
                  You Are Ordering {myOrders?.length} Service
                </h3>
                ...
                <div className="w-full flex  justify-center items-center">
                  ...
                  <div className="modal modal-bottom sm:modal-middle">
                    <div className="bg-gray-100 modal-box relative">
                      <label
                        htmlFor="my-modal-6"
                        className="btn-circle  btn-sm  absolute text-2xl right-2 top-3"
                      >
                        ✕
                      </label>
                      <div className="">
                        <div className="flex flex-col ">
                          <h1 className="font-bold capitalize text-3xl my-4 text-rose-600">
                            Make a Payment
                          </h1>
                        </div>
